...
- After PR/MR merge, run merged worktree cleanup for agent-created experiment folders; remove only clean worktrees whose HEAD is already merged into the base ref, and leave dirty/current/protected worktrees alone. Delete local branch refs only with an explicit cleanup option.
